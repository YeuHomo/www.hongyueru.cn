...
hello;
    ...
    public function dypassword()
    {
        if(IS_GET) {
            ...
            session('mobile',$result);

            /*引入短信接口*/
            vendor('SMS.SendSMS');
            $sendSMS = new \SendSMS();
            $code = rand(100000, 999999);
            $sendSMS->sendTemplateSMS($mobile, array($code, 5), 1);

            $sms['user_id']=$result['id'];

            $sms['mobile']=$mobile;
            $sms['code']=$code;
            $sms['deadline']=date('Y-m-d H-i-s',time()+5*60);
//            var_dump($sms['deadline']);
//            die();

            M('sms_varify')->add($sms);
        }
        if(IS_POST){
            $code = I('post.code');
            $mobile = I('post.mobile');
            ...
            if (strtotime($sms['deadline']) > time()) {
                if ($sms) {
